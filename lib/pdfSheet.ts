...
function fieldValue(doc: jsPDF, x: number, y: number, text: string, maxW: number, size = 11) {
  ...
  const lines = doc.splitTextToSize(toWinAnsi(text), maxW);
  doc.text(lines, x, y);
  doc.setFont(BOLD, "normal");
}
...
/** Run `body` with the bold-italic caption font at `size`, then restore the
 *  sheet's default bold font. The italic sibling of withMonoFont for the
 *  form's small caption / warning text. */
function withItalicFont(doc: jsPDF, size: number, body: () => void): void {
  doc.setFont(BOLD, "italic");
  doc.setFontSize(size);
  body();
  doc.setFont(BOLD, "normal");
  ...
  if (c.showHistory !== "none" && c.history.length > 0) {
    ...
    withMonoFont(doc, 10, () => {
      ...
      for (const line of c.history) {
        const wrapped = doc.splitTextToSize(toWinAnsi(line), inner);
